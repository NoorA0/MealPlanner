    ...
    ui(new Ui::EditMeal_AssignedTags)
{
    ...
    // goa: itemize tags already assigned to mealPtr
    ...
    for (auto& tag : assignedTags)
    {
        ...
        else
        {
            ...
            itemStr += "\nEnabled on: " + CreateDaysEnabledString(tag->getEnabledDays());
        }
        ...
        else
        {
            ...
            while (!done && itemIter != itemsInOrder_Assigned.end())
            {
                ...
                // position found
                if (tag->getName() < compareTag->getName())
                {
                    done = true;
...
QString EditMeal_AssignedTags::CreateDaysEnabledString(const QMap<DaysOfTheWeek, bool> enabledDays)
{
    QString retStr = "[";

...
void MealManager::editMealTags(Meal* mealPtr)
{
    ...
    while (tempStr != "Q")
    {
        ...
        if (mealPtr->getTags().size() == 0)
        {
            ...
            {
                // ui title
                // tell user that meal has no tags, prompt to assign them
                // store answer in tempStr
            ...
            {
                // meal has no tags but no tags exist
            }
        ...
        else // display assigned tags
        {
            ...
            QVector<Tag*> assignedTagsCopy = mealPtr->getTags();
...
void MealManager::editMultiTagTags(MultiTag* mtagPtr)
{
    ...
    while (tempStr != "Q")
    {
        ...
        else // display linked tags
        {
            ...
            // create iterator to linkedTags
            QMap<Tag*, unsigned int> linkedTagsCopy = mtagPtr->getLinkedTags();
        ...
        if (tempStr == "A")
        {
            ...
            do
            {
                ...
                {
                    // exclude Tags already linked
                    QVector<Tag*> linkedTags;
                    if (mtagPtr->getLinkedTags().size() > 0)
                    {
                        QMap<Tag*, unsigned int> linkedTagsMap = mtagPtr->getLinkedTags();
                ...
                {
                    // check if tag already added
                    QMap<Tag*, unsigned int> linkedTags = mtagPtr->getLinkedTags();
...
void MealManager::editMultiTag(MultiTag* mtagPtr)
{
    ...
    while (tempStr != "Q")
    {
        ...
        else if (tempStr != "Q")
        {
            ...
            {
                tempStr = "";
                QMap<DaysOfTheWeek, bool> enabledDays = mtagPtr->getEnabledDays();
...
QString MealManager::formatPrice(const double& price)
{
    QString formatStr = std::to_string(price);

    // erase trailing zeros, leaves '.' at end of string
    formatStr.erase(formatStr.find_last_not_of("0") + 1, QString::npos);
...
int MealManager::displayTags(int& lastPageVisited, Tag*& tagPtr, const QVector<Tag*>& excludeTags)
{
    const int TAGS_PER_PAGE = 5;

    QString tempStr = "";
    QVector<QString> strVec;
    QVector<Tag*> availableTags = normalTags; // make all tags available to display
    int tempInt = -1;
